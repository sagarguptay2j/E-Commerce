...
if(isset($_POST['submit'])){
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			...
			$image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
			move_uploaded_file($_FILES['image']['tmp_name'],PRODUCT_IMAGE_SERVER_PATH.$image);
			mysqli_query($con,"insert into product(categories_id,sel_id,name,mrp,price,qty,short_desc,description,meta_title,meta_desc,meta_keyword,status,image) values('$categories_id','$sel_id','$name','$mrp','$price','$qty','$short_desc','$description','$meta_title','$meta_desc','$meta_keyword',1,'$image')");
		}
		header('location:product.php');
		die();
	}
							...
							<div class="card-body card-block">
							   <div class="form-group">
									...
									<select class="form-control" name="categories_id">
										...
										<?php
										$res=mysqli_query($con,"select id,categories from categories where sel_id=$sel_id order by categories asc");
